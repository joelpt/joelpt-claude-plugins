...
- [ ] Phase 1.4: live BBC Maestro DOM capture — BLOCKING in USER_TODO.md (crawls user's account). Helper script `lib/capture-fixtures.js` shipped: logs in, captures the 6 known-buggy-or-control course pages via `page.content()`, writes `tst/fixtures/<slug>.post-render.html` + a `tst/fixtures/README.md` documenting the post-render capture convention.
