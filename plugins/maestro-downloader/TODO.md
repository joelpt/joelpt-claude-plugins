...
- [WIP] Phase 3: lib/migrate.js (multi-commit). Landed so far:
  ...
  - Pending in follow-up commits: `--copy` (per-course atomic copy with `.copying` staging + size + hasCompletionCues + 5% SHA-256 sample receipts; refuses to run unless re-fetch sentinel passes), `--verify`, `--cleanup`, then wire `migration-lock.blockedBecauseMigrating` into `recordCompletion`.
