...
- [ ] Phase 5: Ink TUI — **DEFERRED to a future session**. Substantial new feature scope (~500 LOC of TUI scaffolding) lower-leverage than the migration work that's already shipped. All prerequisites are in place (Phase 2.5 import-side-effect-free lib modules; `runCourse` / `runQueue` / `runCopy` exportable; `index.json` watchable). Pick this up after the user has run the Phase 1.3 migration + Phase 1.4 fixture capture + Phase 1.5 scraper rewrite — at that point the TUI has actual subscribed/contentType state to manage.
