...
(preference call — `USER_TODO.md`), Linux test (env-gated), and the
`plugin.json` → `0.2.0` + `CLAUDE.md` v2 rewrite at the v2 cutover (the patch
bumps in the cleanup commits are unrelated marketplace housekeeping). The
pre-`--copy` data-integrity review of `migrate.js` is **DONE** — four
correctness fixes (2 critical) landed; see the Phase 3 progress-log entry
below for the C1/C2/H1/H3 outcomes and the H2/M2/M3 accepted-risk rationale.
...
- [x] Phase −1.1: PMS ≥ 1.43.1.10512 — RESOLVED 2026-05-15. User reported build `1.43.1.10611-1e34174b1`; build counter `10611 ≥ 10512` → the NFO Series agent `<namedseason>`/`season.nfo` behavior and the "episodes without ids get no metadata" fix are both present.
- [ ] Phase −1.2: lessonUrl stability spot-check — pending (autonomous; needs Playwright against live account → effectively BLOCKING; tracked under the Phase 1.7/1.8 re-fetch gate in USER_TODO.md).
- [x] Phase 0: Plex `<namedseason>` POC — **✅ GO, verified 2026-05-16** on the user's PMS (build 1.43.1.10611). All `<namedseason>` titles surfaced including the hierarchical `Vocal Exercises → Breathing Fundamentals`/`→ Articulation` subcategory titles; show/episode titles + Specials routing + `poster.jpg` all read from local NFO/assets. Outcome + the agent-default config gotcha recorded in `poc/04-plex-nfo/findings.md`; gotcha baked into `docs/PLEX_SETUP.md`. Artifacts (`generate.mjs` using the production renderers, `findings.md`, `expected-nfo-snapshot.md`) shipped in commit `77a5353`; the `library/` tree is git-ignored and regenerable. `poc/04-plex-nfo/generate.mjs` (uses the **production** `lib/nfo.js`/`lib/layout.js` renderers, so a Plex GO verifies the real `--copy` output by construction), `poc/04-plex-nfo/findings.md` (GO/NO-GO checklist + degraded-path), and `poc/04-plex-nfo/expected-nfo-snapshot.md` (committed rendered XML for review). Synthetic `Sing Like the Stars - Eric Vetro` show: Specials s00 + Season 01 "Lessons" + Season 02 "Vocal Exercises → Breathing Fundamentals" + Season 03 "Vocal Exercises → Articulation"; verified mechanically that `tvshow.nfo` emits all 4 `<namedseason>` tags and `season.nfo` omits `<seasonnumber>`. Generated `library/` tree is git-ignored (binary, regenerable via the committed script). Manual Plex library config + scan + screenshot is on the user.
- [x] Phase 1.4: live BBC Maestro DOM capture — DONE (committed `da1c0e0`). Five post-render fixtures captured via `page.content()` and committed to `tst/fixtures/`: `agatha-christie/writing`, `eric-vetro/singing`, `mark-ronson/music-production`, `oliver-burkeman/time-management`, `owen-o-kane/a-life-less-anxious`. `tst/fixtures/README.md` documents the post-render capture convention. Helper script `lib/capture-fixtures.js` shipped for future re-captures.
- [x] Phase 1.5: `scrapeCoursePage` rewrite (committed `da1c0e0`). Audit `poc/04-phase-1.5-scraper-audit.md` proved BBC Maestro course pages expose NO category structure for downloadable lessons — the old `h2/h3/h4` heading-walk fabricated phantom categories from page-template noise (every fixture has the same 42 template `<h2>`s). Replaced with "always emit one `Lessons` category"; extracted a pure `parseCoursePageHtml(html, courseUrl)` entry point so the scrape logic is TDD-able without a browser. `tst/fetch-list.test.js`: 5 fixture-driven golden tests asserting flat single-category shape + a shape-contract test + slug→instructor derivation test. `eric-vetro/singing`'s separate `/practices/` tab (105 videos, distinct URL pattern, 0 currently in live index) explicitly deferred to Phase 1.6 as a feature decision. **Phase 1.5 fully closed:** `MIGRATION_REQUIRES_REFETCH_AFTER` is now set to the `da1c0e0` commit timestamp (`2026-05-15T02:15:08-07:00`) — audit §3 item 5 done. The `--copy` interlock is now an extracted pure `evaluateRefetchGate({iHaveReFetched, lastFetched, threshold})` (refuses on null threshold, missing/unparseable `lastFetched`, or `lastFetched < threshold`; `--i-have-re-fetched` retained as explicit operator escape hatch). 8 new tests (5 pure-helper + boundary + constant-shape + 2 runCopy integration: stale→refuse, post-1.5→proceed-without-flag); 381 tests pass.
- [ ] Phase 1.3 execution: schema v2 migration of live `index.json` — BLOCKING in USER_TODO.md. Script `lib/migrate-schema-v2.js` shipped with 14 unit tests covering the pure transform, the count assertion, dry-run, real run with backup, refusal on already-v2, and the "live file untouched on validation failure" property.
...
- [WIP] Phase 3: lib/migrate.js (multi-commit). Landed so far:
  ...
  - `lib/migrate.js --copy` + 7 tests — per-course atomic copy with `.copying` staging + fsync + size + `hasCompletionCues` + 5% SHA-256 sample receipts; pre-flight aborts on MISSING_SOURCE; idempotent (skip on existing receipt); writes `tvshow.nfo` + `season.nfo` + per-episode `.nfo`; updates `localPath` in index.json per course (resumable). **Re-fetch interlock now live** (Phase 1.5 closed): `evaluateRefetchGate` refuses unless the index's `lastFetched` is >= the `MIGRATION_REQUIRES_REFETCH_AFTER` Phase 1.5 ship timestamp, with `--i-have-re-fetched` as an explicit operator override. Acquires migration lock; installs SIGINT/SIGTERM release handler.
  ...
  - Lockfile→`recordCompletion` wire-up done: `recordCompletion` now calls `blockedBecauseMigrating(root)` (deriving `root` from `dirname(indexPath)`) before any write; throws on a live lock; passes through on a stale one. 2 new integration tests on `download.test.js` confirm both branches.
  - [x] **Pre-`--copy` data-integrity review DONE** (2026-05-16, autonomous, undeferred ahead of the human go/no-go since the failure mode is irreversible 40 GB loss). A focused data-integrity reviewer + 5 new red→green tests landed four fixes in `lib/migrate.js`:
    - **C2 (critical)** SIGKILL between `writeReceipt` and `atomicWriteJson` stranded `localPath` permanently → `--cleanup` would then delete the only copy. Fix: the idempotency-skip path now `reapplyLocalPathsFromReceipt(course, prior)` so the resumable caller re-publishes `localPath`; receipt actions now carry `lessonUrl`.
    - **H1 (high)** torn `writeReceipt` → parse-fail → idempotency misfires → full re-copy. Fix: new `atomicWriteFileSync` (tmp + fsync + rename); receipt + all three NFO writes routed through it (also closes M1, partial-XML Plex cache poisoning).
    - **C1 (critical)** `--cleanup` deleted v1 sources without re-checking the v2 destination still exists + matches recorded size (`verifiedAt` only proves goodness *at verify time*). Fix: `cleanupCourse` hard-throws (all-or-nothing per course) before unlinking a single source if any destination is missing/wrong-size.
    - **H3 (high)** sampled SHA-256 hashed the destination only (snapshots whatever landed). Fix: hash both ends at copy time and assert equality, throwing on mismatch; `copyCourse` gained a `sampleFraction` test seam.
    - **Accepted risk, NOT fixed (rationale recorded here):** H2 PID-reuse false-positive on the migration lock, M2 stale-lock-clear TOCTOU, M3 `blockedBecauseMigrating`/acquire non-atomicity. All three are false-*blocks* (refuse to run), not data loss; recovery is the already-printed "remove the stale lock at `<path>`" instruction; this is a single-operator CLI where concurrent invocation requires intent. A robust cross-platform process-age check (`ps -o lstart` / `/proc`) is fragile relative to its value and was deliberately not added in an irreversible-data context where the data-loss bugs were the real prize. Revisit only if multi-host/concurrent migration ever becomes a real workflow.
- [x] Phase 4: SPA + serve.js layout-agnostic rewrite. `lib/serve.js` intercepts `GET /index.json`, walking the catalogue tree (both v1 cat.videos and v2 cat.subcategories[].videos) to rewrite each `video.localPath` from absolute filesystem path → URL path relative to root, URL-encoding segments. `ui/index.html` now reads `video.localPath` directly via a single `videoUrl(v) => v.localPath` helper (no more client-side path computation that has to mirror the server's sanitize rules). The category renderer recurses into subcategories with `"Parent → Child"` titles. Handles both shapes seamlessly — old v1 downloads at `<root>/courses/...` still resolve; new v2 downloads at `<root>/<Show> - <Inst>/Season NN/...` resolve. 7 new serve tests including a full HTTP `GET /index.json` roundtrip asserting URL rewriting fires.
- [x] Phase 6 (partial): `docs/PLEX_SETUP.md` written — Plex Web library config checklist with PMS .10512 prereq, the two load-bearing checkboxes ("Use local Assets" + "Use Season Titles"), expected results, Jellyfin/Kodi cross-compat note, degraded-path documentation. `README.md` updated to surface the v2-in-flight summary near the top. **Deferred**: plugin.json#version bump (stays 0.1.9 until the user actually exercises the v2 code paths post-migration) and `CLAUDE.md` updates (the current CLAUDE.md is still accurate for the v1 plugin behavior; v2 specifics are documented in the worktree's TODO.md until the user opts in to the new code).
- [x] Phase 5: Ink TUI shipped — `lib/tui/main.js` single-screen subprocess-only TUI per the trimmed plan scope. Columns: ☑ Subscribed, Title, Instructor, Type, Progress (done/total). Keys: ↑↓ navigate, Space toggle subscribed, t cycle contentType, r rescan (spawns `node lib/fetch-list.js`), d download (spawns `node lib/queue.js`), Esc SIGTERM the subprocess, q quit. Bottom panel tails subprocess stdout/stderr (last 10 lines). `fs.watch` on index.json with 200ms debounce picks up external writes. State mutations go through schema-validated `atomicWriteJson`. 10 unit tests via `ink-testing-library` cover all keyboard handlers and column rendering. `just run` now opens the TUI; `just queue` keeps the legacy non-TUI download runner. `commands/tui.md` exposes the TUI as a slash command.
